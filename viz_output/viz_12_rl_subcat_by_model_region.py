#!/usr/bin/env python3
"""
Visualization 12: Response Length Subcategory Breakdown by Model & Region
"""
import os
import sys
import pandas as pd
...
def main():
    ...
    output_image_path = os.path.join(output_dir, 'viz_12_rl_subcat_breakdown.png')
    output_data_path = os.path.join(output_dir, 'viz_12_data.csv')
    ...
    try:
        ...
        # Identify RL Subcategory Column
        RL_COL = "rl_subcategory"
        if RL_COL not in df.columns:
            for c in df.columns:
                if "rl" in c.lower() and "sub" in c.lower(): 
                    RL_COL = c
                    break
        
        if RL_COL not in df.columns:
            print("Viz 12: 'rl_subcategory' column not found. Skipping.")
            return

        df['model_display'] = df['model_name'].replace({'model_a': 'Base', 'model_b': 'Test', 'Model A': 'Base', 'Model B': 'Test'})

        # Filter: Only Major RL Issues (absolute value == 2, usually)
        failures = df[df['response_length'].abs() == 2].copy()
        
        if failures.empty:
            print("Viz 12: No major Response Length failures found.")
            return

        failures[RL_COL] = failures[RL_COL].fillna("Uncategorized").astype(str)
        
        overall_df = failures.copy()
        overall_df['region'] = 'Overall'
        combined = pd.concat([failures, overall_df], ignore_index=True)

        grouped = combined.groupby(['region', 'model_display', RL_COL]).size().reset_index(name='count')
        ...
        models = ['Base', 'Test']
        subcats = sorted(grouped[RL_COL].unique())
        ...
        for i, region in enumerate(unique_regions):
            ...
            for subcat in subcats:
                vals = []
                for m in models:
                    row = region_data[(region_data['model_display'] == m) & (region_data[RL_COL] == subcat)]
            ...
            ax.set_ylim(0, 115)
            if i % cols == 0: ax.set_ylabel("% of RL Failures")
            ax.grid(axis='y', linestyle='--', alpha=0.3)

        for j in range(i + 1, len(axes)): axes[j].axis('off')

        handles, labels = axes[0].get_legend_handles_labels()
        fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.92), ncol=min(len(subcats), 4), title="Response Length Subcategory", frameon=True)
        ...
        print(f"Error in viz_12: {e}", file=sys.stderr)
        sys.exit(1)
