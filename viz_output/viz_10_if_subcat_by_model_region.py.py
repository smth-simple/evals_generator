#!/usr/bin/env python3
"""
Visualization 10: Instruction Following Subcategory Breakdown by Model & Region
"""
import os
import sys
import pandas as pd
...
def main():
    ...
    output_image_path = os.path.join(output_dir, 'viz_10_if_subcat_breakdown.png')
    output_data_path = os.path.join(output_dir, 'viz_10_data.csv')
    ...
    try:
        ...
        # Identify IF Subcategory Column
        IF_COL = "if_subcategory"
        if IF_COL not in df.columns:
            for c in df.columns:
                if "if" in c.lower() and "sub" in c.lower(): 
                    IF_COL = c
                    break
        
        if IF_COL not in df.columns:
            print("Viz 10: 'if_subcategory' column not found. Skipping.")
            return

        df['model_display'] = df['model_name'].replace({'model_a': 'Base', 'model_b': 'Test', 'Model A': 'Base', 'Model B': 'Test'})

        # Filter: Only Major IF Issues
        failures = df[df["instruction_following"] == 1].copy()
        
        if failures.empty:
            print("Viz 10: No major IF failures found.")
            return

        failures[IF_COL] = failures[IF_COL].fillna("Uncategorized").astype(str)
        ...
        # Aggregation
        grouped = combined.groupby(['region', 'model_display', IF_COL]).size().reset_index(name='count')
        ...
        models = ['Base', 'Test']
        subcats = sorted(grouped[IF_COL].unique())
        ...
        for i, region in enumerate(unique_regions):
            ...
            for subcat in subcats:
                vals = []
                for m in models:
                    row = region_data[(region_data['model_display'] == m) & (region_data[IF_COL] == subcat)]
            ...
            ax.set_ylim(0, 115)
            if i % cols == 0: ax.set_ylabel("% of IF Failures")
            ax.grid(axis='y', linestyle='--', alpha=0.3)

        for j in range(i + 1, len(axes)): axes[j].axis('off')

        handles, labels = axes[0].get_legend_handles_labels()
        fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.92), ncol=min(len(subcats), 4), title="Instruction Following Subcategory", frameon=True)
        ...
        print(f"Error in viz_10: {e}", file=sys.stderr)
        sys.exit(1)
